...
bool driverless_msgs__msg__mpc_centerline__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp(
        "driverless_msgs.msg._mpc_centerline.MPCCenterline",
        full_classname_dest, 49) == 0);
  }
  driverless_msgs__msg__MPCCenterline * ros_message = _ros_message;
  {  // path
    ...
    if (!driverless_msgs__msg__MPCPathPoint__Sequence__init(&(ros_message->path), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create driverless_msgs__msg__MPCPathPoint__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    driverless_msgs__msg__MPCPathPoint * dest = ros_message->path.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!driverless_msgs__msg__mpc_path_point__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of MPCCenterline */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("driverless_msgs.msg._mpc_centerline");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "MPCCenterline");
  ...
  {  // path
    ...
    driverless_msgs__msg__MPCPathPoint * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->path.data[i]);
      PyObject * pyitem = driverless_msgs__msg__mpc_path_point__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
